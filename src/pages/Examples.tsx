    ...
    <div className="min-h-screen pt-16">
      ...
      <section className="py-16 sm:py-20">
        <div className="mx-auto grid max-w-[1400px] gap-6 px-4 sm:px-6 lg:grid-cols-3 lg:px-8">
          {examples.map((example) => (
            <article key={example.title} className="min-w-0 rounded-2xl border border-border bg-background p-5 shadow-sm">
      ...
      <section className="border-y border-border bg-muted/30 py-16">
        <div className="mx-auto flex max-w-5xl flex-col items-start justify-between gap-6 px-4 sm:px-6 md:flex-row md:items-center lg:px-8">
          ...
          <Link to="/docs/examples/complete-application">
            <Button className="gap-2 whitespace-nowrap">Open the guide <ArrowRight className="h-4 w-4" /></Button>
          </Link>
        </div>
      </section>
    </div>
