    ...
    >
      <Section id="application-api" title="Application and framework">
        <ReferenceTable rows={[
          ...
          { name: 'OnInitExpressXApp.useExpressJson(options?)', signature: 'this', description: 'Registers the built-in Express JSON body parser.', notes: 'Accepts ExpressJsonOptions and remains chainable.' },
          { name: 'OnInitExpressXApp.useHelmet(options?)', signature: 'this', description: 'Registers Helmet security headers.', notes: 'Accepts readonly HelmetOptions and remains chainable.' },
          { name: 'OnInitExpressXApp.useUrlencoded(options?)', signature: 'this', description: 'Registers the built-in Express URL-encoded body parser.', notes: 'Accepts ExpressUrlencodedOptions and remains chainable.' },
          { name: 'OnInitExpressXApp.useCors(options?)', signature: 'this', description: 'Registers CORS middleware.', notes: 'Accepts CorsOptions and remains chainable.' },
          ...
          { name: 'Request / Response / NextFn', signature: 'Express type aliases', description: 'Re-exported Express request, response, and NextFunction types.', notes: 'NextFn is used by Express handlers and ExpressX route middleware.' },
      ...
      <Section id="pipeline-api" title="Request pipeline">
        <ReferenceTable rows={[
          ...
          { name: 'ExpressXMiddleware', signature: 'abstract use(ctx, next: NextFn)', description: 'Base class for route-specific middleware.', notes: 'Call next() to continue or next(error) to enter the Express error pipeline; no return is required.' },
      ...
      <Section id="response-api" title="Responses and errors">
        <ReferenceTable rows={[
          ...
          { name: 'return HttpErrorResponse', signature: 'local error path', description: 'Handles an expected failure in the controller or catch block.', notes: 'Remains a transformable interceptor value and uses standard serialization.' },
          { name: 'throw Error', signature: 'application error path', description: 'Rejects the route pipeline and invokes the application ExceptionHandler after interceptors unwind.', notes: 'Recommended when error mapping belongs in the centralized application policy.' },
          { name: 'next(error)', signature: 'Express error path', description: 'Delegates directly to the mounted Express fallback.', notes: 'Bypasses the application ExceptionHandler and standard serializer; use deliberately.' },
...
const issues = [
  ...
  ['req.body is undefined', 'Register useExpressJson() in application.onInit before routes. Confirm Content-Type is application/json.'],
  ...
  ['Global exception handler does not receive an error', 'Errors sent with next(error) intentionally bypass the application handler and standard serializer. Throw the error when it should use the application ExceptionHandler; reserve next(error) for the Express fallback path.'],
